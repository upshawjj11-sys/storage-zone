...
import { ArrowLeft, Save, Plus, Trash2, ChevronDown, ChevronUp, Eye, EyeOff, Upload, X, Globe, Image, AlignLeft, AlignCenter, AlignRight, Bold, Italic, Underline, List } from "lucide-react";
...
function TypographyPanel({ data, onChange, prefix = "" }) {
  const k = (key) => prefix + key;
  return (
    <div className="p-4 bg-blue-50/50 border border-blue-100 rounded-xl space-y-3">
      <p className="text-xs font-semibold text-blue-700 uppercase tracking-wider">Text Styling</p>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-xs">Font Size</Label>
          <Select value={data[k("font_size")] || "base"} onValueChange={(v) => onChange({ [k("font_size")]: v })}>
        ...
        <div>
          ...
          <Select value={data[k("font_weight")] || "normal"} onValueChange={(v) => onChange({ [k("font_weight")]: v })}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <div className="flex gap-1 mt-1">
            {[["left", <AlignLeft className="w-3.5 h-3.5" />], ["center", <AlignCenter className="w-3.5 h-3.5" />], ["right", <AlignRight className="w-3.5 h-3.5" />]].map(([val, icon]) => (
              <button key={val} type="button" onClick={() => onChange({ [k("text_align")]: val })}
                className={`flex-1 h-8 rounded border flex items-center justify-center transition ${data[k("text_align")] === val ? "bg-[#1B365D] text-white border-[#1B365D]" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}>
                {icon}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex gap-1 mt-1">
            {[["italic", <Italic className="w-3.5 h-3.5" />, "italic"], ["underline", <Underline className="w-3.5 h-3.5" />, "underline"]].map(([key, icon, label]) => (
              <button key={key} type="button" onClick={() => onChange({ [k(key)]: !data[k(key)] })}
                className={`flex-1 h-8 rounded border flex items-center justify-center transition ${data[k(key)] ? "bg-[#1B365D] text-white border-[#1B365D]" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}>
                {icon}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <Select value={data[k("list_style")] || "none"} onValueChange={(v) => onChange({ [k("list_style")]: v })}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={data[k("line_height")] || "normal"} onValueChange={(v) => onChange({ [k("line_height")]: v })}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={data[k("letter_spacing")] || "normal"} onValueChange={(v) => onChange({ [k("letter_spacing")]: v })}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={data[k("indent")] || "none"} onValueChange={(v) => onChange({ [k("indent")]: v })}>
            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="none">None</SelectItem>
      ...
      <ColorRow label="Text Color" value={data[k("text_color")]} defaultVal="#374151" onChange={(v) => onChange({ [k("text_color")]: v })} />
    </div>
  );
}
